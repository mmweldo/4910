...
<head>
  ...
  <body>
	...
	<?php
	session_start();
	if(isset($_SESSION['username'])) echo '<form id="logout-form" style="background-color: #2c2c2c; position: relative; top: -13px; right: 10px; width=5%; float:right;" action="logout.php" method="POST"></form><button type="submit" name="submit" form="logout-form">Log Out</button>';
	  ...
	  <div style=" font-size: 1.1rem; font-weight: 400; height: 4vh; position: relative; right: 10px; top: -13px;">
            <img style="border-radius: 50%; right: 0px;" src="'.$row[0].'" width="28" height="28" class="img-circle"></a>
            <p style="right:0px;">&nbsp&nbsp'.$_SESSION['username'].'&nbsp&nbsp</p></div>';
        }
      ?>

      <div class="container">
        <div id="branding">
          <h1><span class="highlight" style="color: #E8BD57;">Drewp:</span> <u>D</u>river <u>REW</u>ards <u>P</u>rogram</h1>
        </div>
        <nav>
          <ul>
            <li><a href="/">Home</a></li>
