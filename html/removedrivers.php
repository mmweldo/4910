<?php  
	...
	if($_SESSION['user_type'] == "sponsor"){
		...
		$query = "SELECT sponsor_id FROM driver_list WHERE driver_id = ".$_SESSION['user_id']." AND sponsor_id = ".$_POST['user_id'].";";
		...
		$query = 'DELETE FROM driver_list WHERE driver_id = '.$_SESSION['user_id'].' AND sponsor_id = '.$_POST['user_id'].';';
		$result = mysqli_query($conn, $query);

		$query = 'DELETE FROM applications WHERE driver_id = '.$_SESSION['user_id'].' AND sponsor_id = '.$_POST['user_id'].';';
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "[3] Error: Couldn't remove driver from list...";
			echo "<script>setTimeout(\"location.href = '../viewsponsorshtml.php?COULDNT-DELETE';\", 3000);</script>";
