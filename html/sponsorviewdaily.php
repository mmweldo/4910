<?php
	...
	$query = "SELECT * FROM driver_list WHERE driver_username = '".$_POST['username']."' AND sponsor_id = (SELECT user_id FROM sponsors WHERE company_name = '".$_POST['company_name']."');"; 	
	$result = mysqli_query($conn, $query);
	$resultCheck = mysqli_num_rows($result);
	if($resultCheck < 1){
			echo "Error: Driver not in your driver_list.";
			echo "<script>setTimeout(\"location.href = '../sponsorviewdaily.html?Not_on_driverlist';\", 3000);</script>";
			exit();
	} 
